...
public class Main {
	...
	private static int query(Node root, int i) {
		clearLazy(root);
	...
	private static void addAscent(Node root, int cnt) {
		if (cnt == 0)
			return;
	...
	private static void addDescent(Node root, int cnt) {
		if (cnt == 0)
			return;
	...
	private static void clearLazy(Node root) {
		if (root.lazy == 0)
			return;
	...
	// 设置左右孩子的lazy标记
	private static void setLazy(Node root, int cnt) {
